...
module.exports = async function handler(req, res) {
  ...
  const { poseData, agency = 'hybe', language = 'ko' } = body;
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 200,
        system: `${AGENCY_STYLES[agency] || AGENCY_STYLES.hybe}. 한 문장으로 짧고 명확한 실시간 코칭 피드백을 ${language === 'ko' ? '한국어' : '영어'}로 제공하세요.`,
        messages: [{
          role: 'user',
          content: `관절 분석: ${jointInfo}, 평균 정확도: ${avgAccuracy}%. 지금 이 순간 연습생에게 줄 코칭 한 문장.`,
        }],
      }),
    });
