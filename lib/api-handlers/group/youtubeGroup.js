...
async function searchDancePractice(apiKey, query, limit = 8) {
  ...
  const settled = await Promise.allSettled(
    queries.slice(0, 3).map((q) => ytSearch(apiKey, q, 12)),
  );
  const allIds = settled.flatMap((result) => (result.status === 'fulfilled' ? result.value : []));
...
async function resolveStreamUrl(videoId) {
  const { resolveStreamUrl: resolve } = require('./streamResolver.cjs');
