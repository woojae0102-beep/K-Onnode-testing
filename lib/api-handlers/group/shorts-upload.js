...
async function verifyUser(idToken) {
  ...
  if (!admin) throw new Error(error || 'Firebase Admin Ï¥àÍ∏∞???§Ìå®');
  if (!idToken) throw new Error('idToken???ÑÏöî?©Îãà??');
...
async function getAccount(admin, uid, platform) {
  ...
  if (!doc.exists || !doc.data()?.connected) throw new Error(`${platform} Í≥ÑÏ†ï???∞Îèô?òÏñ¥ ?àÏ? ?äÏäµ?àÎã§.`);
  return { ...doc.data(), _ref: doc.ref };
}

async function fetchVideoBuffer(videoUrl) {
  const res = await fetch(videoUrl);
  if (!res.ok) throw new Error('Firebase Storage ?ÅÏÉÅ ?§Ïö¥Î°úÎìú???§Ìå®?àÏäµ?àÎã§.');
...
async function refreshTikTokAccount(admin, account) {
  ...
  if (!clientKey || !clientSecret) throw new Error('TikTok OAuth ?òÍ≤Ω Î≥Ä?òÍ? ?ÑÏöî?©Îãà??');
  if (!refreshToken) throw new Error('TikTok refresh token???ÜÏäµ?àÎã§. ?§Ïãú ?∞Îèô?¥Ï£º?∏Ïöî.');
  ...
  if (!res.ok || !data.access_token) {
    throw new Error(data.error_description || data.error?.message || data.error || 'TikTok ?†ÌÅ∞ Í∞±Ïã† ?§Ìå®');
  }
...
async function uploadYouTube(account, payload) {
  ...
  if (!accessToken) throw new Error('YouTube access token???ÜÏäµ?àÎã§.');
  ...
  if (!initRes.ok) {
    ...
    throw new Error(`YouTube ?ÖÎ°ú???∏ÏÖò ?ùÏÑ± ?§Ìå®: ${err}`);
  }
  const uploadUrl = initRes.headers.get('location');
  if (!uploadUrl) throw new Error('YouTube resumable upload URL???ÜÏäµ?àÎã§.');
  ...
  if (!uploadRes.ok) throw new Error(data.error?.message || 'YouTube ?ÅÏÉÅ ?ÖÎ°ú???§Ìå®');
  return {
    platform: 'youtube',
    id: data.id,
...
async function uploadInstagram(account, payload) {
  ...
  if (!accessToken || !igUserId) throw new Error('Instagram Business Í≥ÑÏ†ï ?êÎäî ?†ÌÅ∞???ÜÏäµ?àÎã§.');
  ...
  if (!createRes.ok) throw new Error(created.error?.message || 'Instagram Î¶¥Ïä§ Ïª®ÌÖå?¥ÎÑà ?ùÏÑ± ?§Ìå®');
  ...
  for (let i = 0; i < 12; i += 1) {
    ...
    if (status.status_code === 'ERROR') throw new Error('Instagram Î¶¥Ïä§ Ï≤òÎ¶¨ Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.');
  }
  if (!ready) throw new Error('Instagram Î¶¥Ïä§ Ï≤òÎ¶¨ ?úÍ∞Ñ??Ï¥àÍ≥º?òÏóà?µÎãà??');
  ...
  if (!publishRes.ok) throw new Error(published.error?.message || 'Instagram Î¶¥Ïä§ Í≤åÏãú ?§Ìå®');
...
async function uploadTikTok(account, payload) {
  const accessToken = account.accessToken;
  if (!accessToken) throw new Error('TikTok access token???ÜÏäµ?àÎã§.');
  const buffer = await fetchVideoBuffer(payload.videoUrl);
  ...
  if (!initRes.ok || init.error?.code !== 'ok') throw new Error(init.error?.message || 'TikTok ?ÖÎ°ú??Ï¥àÍ∏∞???§Ìå®');
  const uploadUrl = init.data?.upload_url;
  const publishId = init.data?.publish_id;
  if (!uploadUrl) throw new Error('TikTok upload_url???ÜÏäµ?àÎã§.');
  const uploadRes = await fetch(uploadUrl, {
    method: 'PUT',
    headers: {
      'Content-Type': 'video/mp4',
  ...
  if (!uploadRes.ok) throw new Error(`TikTok chunk upload ?§Ìå®: ${await uploadRes.text()}`);
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (!['youtube', 'instagram', 'tiktok'].includes(platform)) {
      return json(res, 400, { error: 'ÏßÄ?êÌïòÏßÄ ?äÎäî ?åÎû´?ºÏûÖ?àÎã§.' });
    }
    if (!body.videoUrl) return json(res, 400, { error: 'videoUrl???ÑÏöî?©Îãà??' });
