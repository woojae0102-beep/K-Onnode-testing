...
async function verifyUser(idToken) {
  ...
  if (!admin) throw new Error(error || 'Firebase Admin ?????ù?');
  if (!idToken) throw new Error('idToken???ù??ù???');
...
async function exchangeYouTubeCode({ code, redirectUri }) {
  ...
  if (!clientId || !clientSecret) throw new Error('Google OAuth ?ù? ??ùù? ?ù??ù???');
  ...
  if (!res.ok) throw new Error(token.error_description || token.error || 'YouTube ?ù? ?? ?ù?');
...
async function exchangeInstagramCode({ code, redirectUri }) {
  ...
  if (!appId || !appSecret) throw new Error('Facebook OAuth ?ù? ??ùù? ?ù??ù???');
  ...
  if (!shortRes.ok) throw new Error(shortToken.error?.message || 'Instagram ?ù? ?? ?ù?');
  ...
  if (!longRes.ok) throw new Error(longToken.error?.message || 'Instagram long-lived ?ù? ?? ?ù?');
...
async function exchangeTikTokCode({ code, redirectUri, codeVerifier }) {
  ...
  if (!clientKey || !clientSecret) throw new Error('TikTok OAuth ?ù? ??ùù? ?ù??ù???');
  ...
  const token = await res.json();
  if (!res.ok) throw new Error(token.error_description || token.error || 'TikTok ?ù? ?? ?ù?');
...
async function handleCallback(req, res) {
  ...
  if (!platform || !code || !redirectUri) return json(res, 400, { error: 'platform, code, redirectUri? ?ù??ù???' });
  let exchanged;
  ...
  else return json(res, 400, { error: '??ù?? ?ù? ?ù??ù??ù?.' });
...
async function handleDisconnect(req, res) {
  ...
  if (!platform) return json(res, 400, { error: 'platform???ù??ù???' });
