...
async function verifyUser(idToken) {
  ...
  if (!admin) {
    const err = new Error(error || 'Firebase Admin unavailable');
    err.statusCode = 503;
    throw err;
  }
  if (!idToken) {
  ...
  try {
    ...
    const err = new Error(verifyErr?.message || 'Invalid idToken');
    err.statusCode = 401;
    throw err;
  }
}
...
async function exchangeYouTubeCode({ code, redirectUri }) {
  ...
  if (!clientId || !clientSecret) throw new Error('Google OAuth ?? ??? ??????');
  ...
  if (!res.ok) throw new Error(token.error_description || token.error || 'YouTube ?? ?? ??');
...
async function exchangeInstagramCode({ code, redirectUri }) {
  ...
  if (!appId || !appSecret) throw new Error('Facebook OAuth ?? ??? ??????');
  ...
  if (!shortRes.ok) throw new Error(shortToken.error?.message || 'Instagram ?? ?? ??');
  ...
  if (!longRes.ok) throw new Error(longToken.error?.message || 'Instagram long-lived ?? ?? ??');
...
async function exchangeTikTokCode({ code, redirectUri, codeVerifier }) {
  ...
  if (!clientKey || !clientSecret) throw new Error('TikTok OAuth ?? ??? ??????');
  ...
  const token = await res.json();
  if (!res.ok) throw new Error(token.error_description || token.error || 'TikTok ?? ?? ??');
...
async function handleCallback(req, res) {
  ...
  if (!platform || !code || !redirectUri) return json(res, 400, { error: 'platform, code, redirectUri? ??????' });
  let exchanged;
  ...
  else return json(res, 400, { error: '???? ?? ??????.' });
...
async function handleDisconnect(req, res) {
  ...
  if (!platform) return json(res, 400, { error: 'platform????????' });
