...
const TRAINER_PERSONAS = {
  liaKim: {
    ...
    label: 'Lia Kim style',
    domains: ['dance'],
    evaluationCriteria: ['line', 'center of gravity', 'rhythm interpretation', 'movement connection', 'expression density'],
    tone: 'Calm, precise, professional dance trainer tone',
    feedbackStyle: 'Explain the movement cause, correction principle, and connected drill',
    priorities: ['fundamentals', 'detail', 'musicality', 'repeatable practice routine'],
  },
  kpopVocalMaster: {
    id: 'kpopVocalMaster',
    label: 'KPOP Vocal Master style',
    domains: ['vocal'],
    evaluationCriteria: ['breath support', 'pitch stability', 'resonance', 'pronunciation', 'emotion delivery'],
    tone: 'Warm vocal master tone with accurate technical language',
    feedbackStyle: 'Break down why the sound is unstable into breath, vocal fold, and resonance causes',
    priorities: ['breath', 'pitch', 'tone', 'section-based practice task'],
  },
  yg: {
    id: 'yg',
    label: 'YG style',
    domains: ['dance', 'vocal', 'audition'],
    evaluationCriteria: ['style', 'groove', 'charisma', 'identity', 'stage control'],
    tone: 'Direct, energetic producer-style tone',
    feedbackStyle: 'Emphasize what looks weak and what kills the stage presence',
    priorities: ['identity', 'confidence', 'groove', 'killing line'],
  },
  jyp: {
    id: 'jyp',
    label: 'JYP style',
    domains: ['dance', 'vocal', 'audition'],
    evaluationCriteria: ['fundamentals', 'timing', 'live stability', 'attitude', 'sincerity'],
    tone: 'Accurate, realistic trainer and evaluator tone',
    feedbackStyle: 'Emphasize trainability and repeatable correction over raw score',
    priorities: ['timing', 'breath', 'accuracy', 'routine'],
  },
  starship: {
    id: 'starship',
    label: 'Starship style',
    domains: ['dance', 'vocal', 'audition'],
    evaluationCriteria: ['potential', 'team balance', 'stability', 'visual impact', 'consistency'],
    tone: 'Bright, practical tone that finds the growth line',
    feedbackStyle: 'Suggest how to look better inside the team while keeping stable performance',
    priorities: ['potential', 'teamwork', 'stability', 'concept fit'],
  },
};
...
function inferDomain(text = '') {
  ...
  if (hasAny(['vocal', 'sing', 'voice', 'pitch', 'breath', '??', '??', '??', '??', '??'])) return 'vocal';
  if (hasAny(['korean', 'pronunciation', '??', '???', '??', '??'])) return 'korean';
  if (hasAny(['audition', 'judge', '??', '???'])) return 'audition';
  return 'dance';
}
...
async function extractTrainingKnowledge({ transcript = '', metadata = {}, domain = '' }) {
  ...
  const prompt = `K-POP ${resolvedDomain} ?????? ?????? ??? ??????????? ???????
...
async function getDb() {
  ...
  if (!admin) throw new Error(error || 'Firebase Admin ???????');
...
async function transcribeAudioFromUrl(audioUrl) {
  ...
  if (!audioRes.ok) throw new Error('audioUrl ????? ???');
...
function formatRagContext(results = []) {
  return results.slice(0, 5).map((r, i) => {
    ...
    return `[${i + 1}] ${sanitizeText(r.title || r.skill || 'Training knowledge', 100)}\n${sanitizeText(text, 900)}`;
  }).join('\n\n');
}
...
function buildTrainerSystemPrompt({ persona, domain, contextText }) {
  return `????? ONNODE Trainer Knowledge Engine??${persona.label} AI ???????
????? ${domain}
???? ???: ${persona.evaluationCriteria.join(', ')}
??: ${persona.tone}
